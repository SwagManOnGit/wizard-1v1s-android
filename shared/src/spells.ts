...
/**
 * Everything the codex lists, plus the apocrypha unsealed at load.
 *
 * The unlisted spells live in secrets.data.ts as an opaque blob rather than as readable entries in
 * this file, because "Black Sun" next to `glyph: 'sunburst', reverse: true` was the whole secret in
 * plain text in the shipped bundle. See secrets.ts for what that is and is not worth.
 */
export const SPELLS: SpellDef[] = [...LISTED];
SPELLS.push(...unseal());
...
export const LISTED_SPELLS = LISTED.length;
