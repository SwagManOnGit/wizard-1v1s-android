...
**Guard the secrets in the build you do ship.** The spell table is plain TypeScript in the web
bundle, so the first curious player can read all 15 unlisted spells out of the JavaScript within an
hour of launch. If the apocrypha are meant to last months, the glyph templates for unlisted spells
should be stored as hashes of the normalised point set, with the shape reconstructed only on a
successful match — or held server-side and validated on cast. Without that, section 7 has a shelf
life of one afternoon.
